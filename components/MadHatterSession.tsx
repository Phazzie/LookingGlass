...
export function MadHatterSession({ focusTimeMinutes, onSetFocusTimeMinutes, onConsult }: MadHatterSessionProps) {
  return (
      ...
      <div className="w-full max-w-xs bg-purple-900/25 border border-amber-500/10 rounded-xl p-4.5 mb-5 space-y-3.5 text-left">
        ...
        <div className="space-y-1">
          <input
            ...
            onChange={(e) => onSetFocusTimeMinutes(parseInt(e.target.value, 10))}
        ...
        <div className="grid grid-cols-4 gap-1.5 pt-1">
          {[5, 10, 15, 20].map((t) => (
            <button
              key={t}
              type="button"
              onClick={() => onSetFocusTimeMinutes(t)}
